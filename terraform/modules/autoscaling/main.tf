...
resource "aws_autoscaling_group" "asg" {
  ...
  launch_configuration = var.create_lc ? element(concat(aws_launch_configuration.lc.*.name, [""]), 0) : var.launch_configuration
...
resource "random_pet" "asg_name" {
  ...
  keepers = {
    ...
    lc_name = var.create_lc ? element(concat(aws_launch_configuration.lc.*.name, [""]), 0) : var.launch_configuration
  }
}
